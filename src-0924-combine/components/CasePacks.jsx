...
import './use-case.css';
import {SceneUIContext} from '../usecase-src/components/SceneUI';
...
class SceneBoundary extends Component {
  ...
  render() {
    return this.state.failed
      ? <div className="case-scene-fallback">OP<span>{this.props.lang==='en'?'HR Agent is ready':'HR Agent 隨時待命'}</span></div>
      : this.props.children;
  }
}
...
function CaseScene({ lang, kind, progress, entry, reduced, onReadyChange }) {
  ...
  return <SceneUIContext.Provider value={overlay}><div className={`case-scene${(['overtime','dispatch','leave','contract','punch'].includes(kind)) ? ' case-scene--modeled' : ''}`} ref={host} aria-hidden="true">
    {near && <SceneBoundary lang={lang}>
      <Suspense fallback={<div className="case-scene-fallback">OP<span>{lang==='en'?'Preparing your HR workspace':'正在準備 HR 工作空間'}</span></div>}>
        {(['overtime','dispatch','leave','contract','punch'].includes(kind))
          ? <ModeledHROffice lang={lang} onReadyChange={onReadyChange} scenario={kind} />
...
const englishHeadings=[['Watch overtime.','Act early.'],['Send tasks.','Track replies.'],['Leave expires.','Plan ahead.'],['Dates matter.','Stay ahead.'],['Missed a punch?','Just ask.']];
const englishTitles=['Overtime alerts','Attendance tasks','Leave expiry','Contract expiry','Missed punches'];
const englishBlurbs=['Weekly scans flag overtime. LINE alerts reach staff and managers.','Send gaps via LINE. Track only open tasks.','90-day reminders help teams plan leave.','30-day reminders for probation and contracts.','One LINE message. AI checks records and sends a summary for review.'];
export default function CasePacks({ packs, handoff }) {
  const {lang}=useLang(),en=lang==='en';
  const headings=en?englishHeadings:scenarioHeadings,blurbs=en?englishBlurbs:scenarioBlurbs;
  ...
  playing.current = onScreen.current && ready && (handoff?.current.settled ?? true);
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        playing.current = onScreen.current && readyRef.current && (handoff?.current.settled ?? true);
    ...
    window.addEventListener('office-handoff',measure);
    return () => {
      cancelAnimationFrame(frame);
      motion.removeEventListener('change', motionChange);
      window.removeEventListener('scroll', measure);
      window.removeEventListener('resize', measure);
      window.removeEventListener('office-handoff',measure);
    };
  }, [count,handoff]);
  ...
  return <ScenarioClock active scenario={active} replay={replay} playing={playing} onComplete={advance} entry={entry} reduced={reduced}>
    <div className={`use-case-explorer case-journey${active < 5 ? ' case-journey--modeled' : ''}`} ref={editorial}>
      <div className={`case-stage case-stage--${scenario.visual}`} ref={stage}>
        ...
        <CaseScene lang={lang} onReadyChange={setReady} kind={scenario.visual} progress={index} entry={entry} reduced={reduced} />
        <div className="case-editorial">
          <header className="case-masthead">
            <span>USE CASES</span>
            <nav className="case-pack-label" aria-label={en?"Department packs":"部門 Pack"}>
              {packs.map((pack) => <button key={pack.id} disabled={pack.status === 'soon'} aria-current={pack.id === live.id ? 'true' : undefined}>
                {en?({hr:'HR',finance:'Finance',sales:'Sales'}[pack.id]+' Pack'):pack.label}{pack.status === 'soon' && <small>{en?'Coming soon':'即將推出'}</small>}
              </button>)}
            </nav>
          </header>
          <ol className="case-accordion" aria-label={en?"HR scenarios":"HR 情境導覽"}>
            {live.scenarios.map((one, i) => {
              ...
              return <li key={one.visual} className={open ? 'is-active' : i < active ? 'is-complete' : ''}>
                <button onClick={() => choose(i)} aria-current={open ? 'step' : undefined} aria-expanded={open}>
                  <span>0{i + 1}</span>{en?englishTitles[i]:one.title.replace('試用期／合約到期提醒', '合約到期提醒')}
                </button>
                <div className="case-accordion-body" aria-hidden={!open}>
                  <div className="case-copy-inner" aria-live={open ? 'polite' : undefined}>
                    <h2>{headings[i][0]}<span>{headings[i][1]}</span></h2>
                    <p className="case-description">{blurbs[i]}</p>
                  </div>
                </div>
              </li>;
            })}
