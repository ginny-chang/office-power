...
function Monitor({kind}){
 ...
 useEffect(()=>{
  ...
  // A clock and a grid of people replace the schedule paragraphs.
  ctx.strokeStyle='#b9c8d9';ctx.lineWidth=3;ctx.beginPath();ctx.arc(48,43,19,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.moveTo(48,28);ctx.lineTo(48,43);ctx.lineTo(34,43);ctx.stroke();
  ctx.fillStyle='#c8d5e2';ctx.font='22px sans-serif';ctx.fillText('09:00',82,51);
  for(let i=0;i<18;i++){ctx.fillStyle=time>=18?'#88ad97':'#7e94aa';ctx.beginPath();ctx.arc(310+i%9*32,30+Math.floor(i/9)*27,5,0,Math.PI*2);ctx.fill();ctx.fillRect(304+i%9*32,36+Math.floor(i/9)*27,12,5);}
  if(time>=5){
   const warningY=353-40/50*230;
   // A shaded warning zone and bright caps show exactly which hours exceed 40.
   ctx.fillStyle='#f5ad241c';ctx.fillRect(25,105,550,warningY-105);
   [27,33,41.5,44].forEach((h,i)=>{
    const v=h*Math.min(1,Math.max(0,(time-5-i*.45)/.8)),x=45+i*132,y=353-v/50*230;
    ctx.fillStyle=v>40?'#d3a45b':'#829ab5';ctx.fillRect(x,y,68,v/50*230);
    if(v>40){ctx.save();ctx.shadowColor='#ffb52e';ctx.shadowBlur=15;ctx.fillStyle='#ffc247';ctx.fillRect(x,y,68,(v-40)/50*230);ctx.restore();ctx.strokeStyle='#ffe5a2';ctx.lineWidth=2;ctx.strokeRect(x,y,68,(v-40)/50*230);}
    ctx.font='bold 23px sans-serif';ctx.fillStyle=h>40?'#ffd477':'#f1f3f7';ctx.fillText(h+'h',x,380);
    if(v>40){ctx.font='bold 22px sans-serif';ctx.fillText('+'+(h-40)+'h',x,108);}
   });
   ctx.strokeStyle='#ffc247';ctx.lineWidth=4;ctx.setLineDash([10,5]);ctx.beginPath();ctx.moveTo(25,warningY);ctx.lineTo(575,warningY);ctx.stroke();ctx.setLineDash([]);
   ctx.fillStyle='#ffc247';ctx.fillRect(555,warningY-17,80,34);ctx.fillStyle='#263342';ctx.font='bold 23px sans-serif';ctx.fillText('40h',565,warningY+8);
   ctx.strokeStyle='#a98790';ctx.lineWidth=1;const limitY=353-46/50*230;ctx.beginPath();ctx.moveTo(25,limitY);ctx.lineTo(575,limitY);ctx.stroke();ctx.fillStyle='#c2a6ae';ctx.font='16px sans-serif';ctx.fillText('46h',583,limitY+5);

  } else {for(let i=0;i<24;i++){ctx.fillStyle=i<time*5?'#a3bead':'#465a70';ctx.beginPath();ctx.arc(60+i%8*74,160+Math.floor(i/8)*70,12,0,Math.PI*2);ctx.fill();ctx.fillRect(44+i%8*74,178+Math.floor(i/8)*70,32,16);}}
...
export default function HROffice({kind='overtime',reduced=false}){
 ...
 return <group>
  ...
  <group position={[.55,1.28,-.83]} rotation={[-Math.PI/2,0,-.2]}><Box size={[.47,.6,.018]} color="#f4f2ed"/>{[0,1,2].map(i=><Box key={i} at={[0,.13-i*.1,.014]} size={[.3,.02,.005]} color={kind==='contract'?'#aa9ab8':'#b5bdc5'}/>)}</group>
  <group ref={stamp} position={[.6,1.36,-.84]}><Box size={[.22,.06,.19]} color="#a194ad"/><Box at={[0,.11,0]} size={[.08,.2,.08]} color="#697583"/></group>
